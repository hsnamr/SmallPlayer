...
//  SmallPlayer
//
//  MEncoder backend: mplayer -identify -frames 0 for W/H, then
//  mencoder <file> -ovc raw -vf format=rgb24 -of rawvideo -o - -nosound
...
@implementation SPMencoderBackend
...
- (NSString *)backendIdentifier { return SPPlayerBackendIdentifierMEncoder; }
- (NSString *)backendDisplayName { return @"MEncoder"; }
...
- (void)readLoop {
    @autoreleasepool {
        ...
        if (![self getVideoSizeForPath:path width:&w height:&h fps:&fps]) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if ([del respondsToSelector:@selector(playerBackend:didFailWithError:)])
                    [del playerBackend:self didFailWithError:@"MEncoder: could not get video size (mplayer -identify)"];
            });
            _playing = NO;
            return;
        }
        ...
        [task setLaunchPath:@"/usr/bin/mencoder"];
        [task setArguments:@[
            [NSString stringWithUTF8String:cpath],
            @"-ovc", @"raw",
            @"-vf", @"format=rgb24",
        ...
        @try {
            ...
            id<SPPlayerBackendDelegate> del = self.delegate;
            NSString *err = [NSString stringWithFormat:@"MEncoder failed: %@", [e reason]];
            dispatch_async(dispatch_get_main_queue(), ^{
                if ([del respondsToSelector:@selector(playerBackend:didFailWithError:)])
